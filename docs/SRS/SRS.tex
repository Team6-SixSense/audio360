...
\begin{enumerate}
  ...
  \item \label{FR1_3}\textbf{FR1.3:} The firmware shall detect and flag audio
  anomalies such as clipping, lost signal and silence.

  \item \label{FR1_4}\textbf{FR1.4:} The firmware shall provide an interface
...
\begin{enumerate}
  ...
  signals in real-time with monotonic frame sequences.
  
  \item \label{NFR1_2}\textbf{NFR1.2:} The firmware shall have a audio
  processing success rate of atleast 90\% over 5 seconds.
  
  \item \label{NFR1_3}\textbf{NFR1.3:} The firmware shall operate
...
\begin{enumerate}
  ...
  thoguht locking when writing to memory to ensure data that is still being
  used is not lost.
\end{enumerate}
...
\begin{enumerate}
  ...
  constraints defined in \href{NFR1_3}{NFR1.3} for input signal size up to
  44100 frames.
\end{enumerate}
...
Functional Requirements
\begin{enumerate}
  \item \label{FR4_1}\textbf{FR4.1:} The program shall retrieve frequency
  domain data from the microcontroller via UART for further processing.
...
\begin{enumerate}
  ...
  conflicting with ongoing microcontroller writes, ensuring no data loss.
...
\begin{enumerate}
  ...
  \item \label{NFR5_2}\textbf{NFR5.2:} The component shall achieve a minimum
  classification accuracy of 90\% under normal operating conditions.

  \item \label{NFR5_3}\textbf{NFR5.3:} The component shall estimate the
  direction of arrival of an audio source with a maximum error of 20 degrees,
...
\begin{enumerate}
  ...
  time because screen space is limited.
