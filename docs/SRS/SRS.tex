...
This section provides detailed versions of the high-level scenarios from G.5, with specific technical details and interaction flows.

\subsubsection{S.4.1 Pedestrian Crossing Detection (Detailed)}
...
\begin{enumerate}
    ...
    \item Smart glasses display visual indicator showing direction and "Vehicle detected"
    \item User recognizes the alert and waits for the vehicle to pass before crossing safely
\end{enumerate}

\subsubsection{S.4.2 Kitchen Safety Alert (Detailed)}
...
\begin{enumerate}
    ...
    \item Smart glasses display directional indicator with "Kettle whistling" (≤1s latency)
    \item User turns toward the alert and removes the kettle from heat, preventing potential hazard
\end{enumerate}

\subsubsection{S.4.3 Social Interaction (Detailed)}
...
\subsubsection{S.4.4 Workplace Awareness (Detailed)}
...
\begin{enumerate}
    ...
    \item Smart glasses display prominent visual indicator with "FORKLIFT BACKING UP" and directional arrow
