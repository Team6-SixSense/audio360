...
\externaldocument[DevelopmentPlan-]{../DevelopmentPlan/DevelopmentPlan}
...
2025-11-05 & 3.0 & Addressing TA comments\\
2025-11-05 & 3.1 & Symbolic Constants section\\
2025-11-05 & 3.2 & Providing additional rationale and clarification for
functional and non-functional requirements.\\
2025-11-05 & 3.3 & Verification and acceptabce criteria abstracted away from
defining specific metrics.\\
2025-11-07 & 4.0 & Added requirements for software layer abstraction (FR4.5) \\
2025-11-12 & 4.1 & Added a symbols, abbreviations, and acronyms table \\
2025-12-12 & 4.2 & Added source for +/- 45 degree accuracy requirement.\\
2026-01-06 & 4.3 & All angles represented in radians.\\
2026-03-11 & 5.0 & Hardware symbols and abbreviations added.\\
2026-03-25 & 5.1 & Updating requirements metrics associated with new 
classification\\
2026-03-26 & 5.2 & Minimal user input requirement \\
\bottomrule
\end{tabularx}
...
\subsection{G.1 Context and overall objective}\label{sec:G.1}
...
as the sound of a siren approaching, a kettle whistling, or a phone ringing may be
...
indications of sound source locations and classifications (ex. 'siren on your 
left').
...
\begin{goal}\label{goal:user_safety} Ensure that the system provides additional
safety aids to the user by providing information to the user to make
cautious decisions. Also ensure user privacy and security when using the system
to avoid external malicious actors.
...
\subsection{G.4 Functionality overview} \label{sec:g4}
...
user's position, with a target accuracy of \hyperref[symbol:theta_e]{$\theta_e$}
for single sound sources.
...
with a target accuracy of at least
\hyperref[symbol:A_classfication]{$A_{classification}\%$}.
...
($\leq$ \hyperref[symbol:t_lat]{$t_{lat}$} second).
...
As they prepare to cross, a siren approaches from their left side. The system
...
hearing loss in Canada alone (1 in 10) \cite{Healthing2025}.
The McMaster Sign Language club is a
...
\begin{itemize}
  ...
  \item \textbf{Euclidean Quadrants:}\label{def:euclidean_quadrant}
  The four regions of a 2D Cartesian plane, divided by the x and y axes,
  numbered 1 to 4 in a counterclockwise direction starting from the top-right.
  The ranges of the quadrants are (0, $\frac{\pi}{2}$),
  ($\frac{\pi}{2}$, $\pi$), ($\pi$, $\frac{3\pi}{2}$), and
  ($\frac{3\pi}{2}$, $2\pi$) radians.

  \item \textbf{Frequency Domain:}\label{def:frequency_domain}
  A way to represent periodic or sinusoidal signals based on their frequency
  content, showing how much of the signal lies within each given frequency
  range. Frequency units is $Hz$.

  \item \textbf{Hardware Acceleration:}\label{def:hardware_acceleration}
  The use of specialized hardware modules within a microcontroller to perform
  specific computational tasks that are more computationally efficient than
  running equaicalent software on the CPU. For example, signal processing and
  vector operations.
  ...
  rate, input audio signals are within the human hearing most audible range
  ...
  \item \textbf{STM32:} \label{def:stm32}
  Family of 32 bit microcontrollers provided by STMicroelectronics
  \cite{STMicroelectronics}.

\end{itemize}

\subsubsection{E.1.1 Symbols, Abbreviations, and Acronyms}
\label{sec:symbols}
...
\begin{longtable}{|l|l|p{10cm}|}
    ...
    \hline
    $\theta_e$ \label{symbol:theta_e} & ±$\frac{\pi}{4}$ rad & 
    Angle error threshold. $\frac{\pi}{4}$ threshold is selected to indicate the
    sound
    source direction within half-quadrant resolution. This is also approximately
    equal to the horizontal movement possible by human eyes \cite{Eye_Hang_Woo} \\
    \hline
    $A_{classification}$ \label{symbol:A_classfication} & $85\%$ & 
    Classification accuracy. $85\%$ is chosen as it an achievable metric
    with the time given while providing high confidence in audio
    classification for stakeholder's use cases. \\
    \hline
    $t_{lat}$ \label{symbol:t_lat} & $1 s$ & 
    Latency time of visual feedback. 1 second was chosen arbitrarily
    chosen. It will be tuned to better fit stakeholder's reaction time
    in critical situations. This will be tuned during the validation
    stage.\\
    \hline
    $N_{mic}$ \label{symbol:N_mic} & $4$ & 
    The number of microphones connected to the microcontroller. 4 is currently 
    chosen because atleast 2 microphones is required for directional analysis,
    and the current microcontroller board supports up to 4 microphones. Tracing
    to \hyperref[NFR9_2]{NFR9.2}\\
    \hline
\caption{Table of symbolic constants used in \progname.}
...
\begin{longtable}{|l|l|} 
    ...
    ICA & Independent Component Analysis \\ \hline
    LDA & Linear Discriminant Analysis \\ \hline
    M & Module \\ \hline
    MG & Module Guide \\ \hline
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    (ex. siren passing by, people talking, dog barking)
...
\subsection{E.3 Constraints} \label{sec:constraints}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item The system must not emit heat at levels that cause harm to the user. 
  \end{itemize}
...
\begin{figure}[H]     
    \centering 
    \includegraphics[width=0.7\textwidth]{diagrams/s1_component_diagram.png}
...
\begin{itemize}
  ...
  its priority. Priority ensures that critical tasks are ran and completed
  before its deadline, avoiding functionality loss.
  
  \item \label{FR1_2}\textbf{FR1.2:} The firmware shall process and synchronize
  audio signals from all microphones connected to the microcontroller. Audio
  synchronization is required to determine direction of audio source as it
  depends on frequency phase shift.

  \item \label{FR1_3}\textbf{FR1.3:} The firmware shall handle memory errors to
  prevent the microcontroller from crashing. This ensures that the system will
  never enter an unrecoverable state.
...
\begin{itemize}
  ...
  in real-time with monotonic sample sequences. Earliest samples shall have
  higher priority. For example, if sample $S_t$ arrives at time $t$, then the
  priority, $P(S_t)$, must satisfy $P(S_t) > P(S_{t+1})$.
  
  \item \label{NFR1_2}\textbf{NFR1.2:} The firmware shall operate at the same
  rate or faster than the connected microphone sample rate of 16 kHz, referring
  to \hyperref[NFR7_1]{NFR7.1}. This ensures that all input data from the
  microphone can be processed.

  \item \label{NFR1_3}\textbf{NFR1.3:} The firmware shall permanently
  discard microphone audio data that is greater than the sampling window size.
  This ensures that audio data is retained only temporarily for processing,
  supporting \hyperref[goal:user_safety]{Goal 7}, protecting user privacy.
\end{itemize}

% DRIVER LAYER REQUIREMENTS.
\subsubsection{\hyperref[comp:driver_layer]{Driver Layer}} Functional
Requirements \label{sec:FR2}
\begin{itemize}
...
\begin{itemize}
  ...
  any errors to the firmware layer for proper error handling. This is to ensure
  that the processor can take the neccessary steps to recover out of the error.
\end{itemize}

% AUDIO FILTERING REQUIREMENTS.
...
\begin{itemize}
  ...
  digital audio waveform to \hyperref[def:frequency_domain]{frequency domain}.

  \item \label{FR3_2}\textbf{FR3.2:} The audio filtering component shall
  normalize the amplitude of incoming and outgoing signals. This is to simplify 
  analysis when comparing different signals.
  ...
  available \hyperref[def:hardware_acceleration]{hardware acceleration}
  for efficient computations. The system needs to run as fast as possible with
  limited compute power, and using hardware acceleration is the most efficient
  method available.
...
\begin{itemize}
  ...
  the true value. This is maximum tolerable error to process audio for
  classification and directional analysis.
  ...
  constraints defined in \hyperref[NFR1_2]{NFR1.2} for input signal that are
  powers of 2 and size up to 4096 samples. Powers of 2 since 
  hardware acceleration, from \hyperref[FR3_4]{FR3.4}, for FFT are most
  optimized for this \cite{CMSIS_DSP_FFT}. 4096 samples is the maximum standard
  size.

  \item \label{NFR3_3}\textbf{NFR3.3:} The audio filtering component shall have
  an audio processing success rate end to end of atleast 80\%.
  Due to variations in input audio quality, some errors are expected.
  While state of the art audio classification systems can reach up to 85\%
  accuracy, they typically require significantly more computational resources
  that this project does not have access to.

\end{itemize}

% AUDIO360 REQUIREMENTS.
...
\begin{itemize}
  ...
  Only processed data should be visualized to the user.
  ...
  \item \label{FR4_5}\textbf{FR4.5:} The Audio360 Engine shall be portable. That
  is the system can be deployed on different hardware. This is to ensure that
  the there is flexibility to upgrade hardware to meet higher processing demands
  when needed.
\end{itemize}
...
\begin{itemize}
  ...
  This adheres to \hyperref[goal:user_safety]{Goal 7}, protecting user privacy.

  \item \label{NFR4_4}\textbf{NFR4.4:} The Audio360 Engine shall minimize 
  required user interactions during software runtime, enabling users to
  perform their tasks with minimal interuptions.
\end{itemize}
...
\begin{itemize}
    ...
    classify sound sources based on features extracted from the
    \hyperref[def:frequency_domain]{frequency domain} representation of the
    audio input signal.
...
\begin{itemize}
  ...
  Three was chosen to provide a minimal base that is useful for the
  stakeholders.

  \item \label{NFR5_2}\textbf{NFR5.2:} The frequency analysis component shall
  achieve a minimum classification accuracy of
  \hyperref[symbol:A_classfication]{$A_{classification}$} under
  ...
  \hyperref[symbol:theta_e]{$\theta_e$} radians. This requirement ensures that
  the system can correctly notify users of sound sources within their
  respective \hyperref[def:euclidean_quadrant]{Euclidean quadrants}.
\end{itemize}

% VISUALIZATION CONTROLLER REQUIREMENTS.
...
\begin{itemize}
  ...
  from its environment and translate them to digital representation.

  \item \label{FR7_2}\textbf{FR7.2:} The microphone shall collect soundwaves
  within the human hearing most audible range (250 Hz - 8 kHz)
  \cite{MiracleEar2025}.
\end{itemize}
...
\begin{itemize}
  ...
  at 16 kHz. According to Nyquist's theorem, audio sampling should be greater
  than two times the highest frequency to avoid aliasing \cite{DataForth2025}.
  The maximum frequency most audible to humans is approximately 8 kHz.
  \cite{MiracleEar2025}.
\end{itemize}
...
\begin{itemize}
  ...
  ensures protection against unauthorized access adhering to
  \hyperref[goal:user_safety]{Goal 7}: security and privacy.
\end{itemize}
...
\begin{itemize}
  ...
  \hyperref[NFR7_1]{NFR7.1}. This allows approximately 5000 computational steps
  to process each audio sample, which is enough to meet core features
  functionality.

  \item \label{NFR9_2}\textbf{NFR9.2:} The microcontroller shall provide atleast
  four microphone inputs to support directional analysis of audio.
  Directional analysis requires a minimum of two microphones. However, since the
  microphones are mounted on glasses and sound damping occurs across the user's
  head, at least four microphones, one in each corner, are required to achieve
  reliable directional analysis.
\end{itemize}
...
street intersection. As they prepare to cross, a siren approaches from their left
side.
...
\begin{enumerate}
    ...
    \item Audio classification identifies it as a kettle or alarm sound (85\%
    accuracy target)
...
\begin{itemize}
    ...
    conversion
    \item \hyperref[FR7_2]{FR7.2} - Human most audible range coverage
      (250 Hz - 8 kHz)
    ...
    \item \hyperref[NFR9_2]{FR9.2} - Four microphone inputs for directional
    analysis
\end{itemize}
...
\begin{itemize}
    ...
    sources
    \item \hyperref[NFR5_2]{NFR5.2} - 85\% classification accuracy
\end{itemize}
...
\begin{itemize}
    ...
    \item \hyperref[NFR5_3]{NFR5.3} - Maximum 
      \hyperref[symbol:theta_e]{$\theta_e$} radian DoA error
\end{itemize}
...
in \hyperref[sec:G.1]{G.1 Context and overall objective} and the
functional/non-functional requirements defined in 
\hyperref[sec:S.2]{S.2 Functionality}.
...
all four microphones in the array capture audio samples with synchronously, that
is all microphone data are captured within 4 timesteps (one timestep per
microphone). This will be verified through oscilloscope measurements or
...
continuous audio capture for the duration the power lifecycle of the system
(single charge on a battery) without buffer overruns, dropped samples, or
system crashes, verified through stress testing and log file analysis.
\end{itemize}
...
an angular error of less than or equal to \hyperref[symbol:theta_e]{$\theta_e$}
in all test cases, verified through testing with known sound source positions
at various angles, $\theta$, that are multiples of $\theta_e$ and satisfies
$0 \leq \theta \leq 2\pi$ rad.

\item \textbf{VC-2.2 Processing latency:} The DoA estimation algorithm shall
complete processing and output a direction estimate within
\hyperref[symbol:t_lat]{$t_{lat}$}  of
...
shall provide consistent direction estimates within
\hyperref[symbol:theta_e]{$\theta_e$}, verified through repeated testing.
\end{itemize}
...
shall achieve at least \hyperref[symbol:A_classfication]{$A_{classification}\%$} 
accuracy in identifying sound categories from a predefined set, verified through
testing with a labeled dataset.

\item \textbf{VC-3.2 Processing latency:}\label{sec:VC-3.2} The classification 
algorithm shall complete processing and output a classification label before
the next microphone audio sampling, verified through timestamp logging.

\item \textbf{VC-3.3 Category coverage:} The system shall support classification
of at least 3 distinct sound categories relevant to safety and daily life,
...
detection to visual display on the smart glasses shall complete within 
\hyperref[symbol:t_lat]{$t_{lat}$}, verified through synchronized
...
device and begin receiving audio alerts without requiring external
documentation, verified through observing participants unfamiliar
with the system using it for the first time.
...
complete initialization and enter operational mode in all power-on
attempts, verified through repeated startup testing. This ensures the system
is working all the time as it is a safety critical system.
\end{itemize}
...
individual work required for deliverables. Admin related roles, and general 
technical subgroups have been outlined under 
\hyperref[DevelopmentPlan-sec:TeamMemberRoles]{Team Member Roles} section of 
the Development Plan. \\

In addition to these responsibilities, each project member
...
defined in functional requirement \hyperref[NFR9_1]{NFR9.1} can be completed
within timing constraints. A microcontroller is also chosen because it can be
physically embedded within the glasses, ensuring low power consumption and
compact integration. For this capstone project, a larger developer
microcontroller will be used for ease of development, regardless of if it can be
embedded into the glasses. Future iterations must use a microcontroller that is
small enough to fit inside the glasses and has the required computer power.
\newline
\newline
\textbf{Microphones:}  
