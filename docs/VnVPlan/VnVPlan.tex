...
2025-10-31 & 2.2 & Metrics included to checklist in section 4.3 \\
...
eye movement\\
2026-01-06 & 3.3 & Update angle units from degrees to radians. \\
2026-02-18 & 3.4 & Integrate TA feedback (primarily removing implementation 
details from test cases). \\
2025-02-18 & 3.5 & Update code coverage tools. \\
2025-03-09 & 4.0 & Test-FR-2.3 Error handling test procedure updated. \\
2025-03-25 & 4.1 & Classification: Updating classification test metrics \\
2025-03-30 & 4.2 & Specifying Software Theory report \\
\hline
\end{tabularx}
...
\begin{itemize}
    \item \textbf{Software Correctness:} Verify that core algorithms 
    (direction-of-arrival estimation and sound-event classification) behave 
    correctly across expected operating conditions. Incorrect localization or 
    missed events could produce misleading guidance, reducing user trust and 
    potentially introducing safety risks identified in the Hazard Analysis.
    
    \item \textbf{Real-time Performance:} Demonstrate that the system provides 
    feedback quickly enough to support live use without confusing or delaying 
    the user. Delayed feedback reduces usability and can invalidate the safety 
    benefit of alerts during real-world interactions (user needs + HA timing 
    considerations).
    ...
    Language Club. User understanding is required for the system to be safe and
    effective; misinterpretation is a failure mode even if algorithms are 
    ...
    failures. Reliability prevents silent failures and increases trust, 
    especially when the system is used as an assistive safety tool.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Software Theory Report:} 
    The software theory report will focus on documenting Direction of Arrival
    analysis and sound classfication theory. The theory will go in depth on how 
...
simultaneously by all four microphones. 
					
\textbf{Output:}
Four synchronized audio buffers, each containing the impulse signal, and a time 
alignment value for each pair of channels. 
...
\textbf{Output:}
For each fault condition, the diagnostic system outputs error codes 
identifying the failed component.

\textbf{Pass Criteria:}
\begin{itemize}
\item Fault detection latency $\leq$ 1 second (measured from physical fault
occurrence timestamp to diagnostic log entry timestamp)
\item Correct error code generated for each fault type (microphone disconnect,
display disconnect, etc.)
\item 100\% fault detection rate (no missed faults)
\item Zero false positives (no spurious error logs when hardware is operational)
\item Recovery detection within 1 second of hardware restoration
\end{itemize}
...
1 second from fault occurrence) ensures users are aware of system limitations.
Component identification in error logs aids in troubleshooting and repair.
					
\textbf{How test will be performed:}
Deploy firmware to microcontroller with all peripherals connected. Start
diagnostic logging with high-precision timestamps. Systematically introduce each
fault condition while recording the exact fault occurrence time:

\begin{enumerate}
\item Record baseline timestamp ($t_0$) immediately before introducing fault
\item Introduce fault (e.g., physically disconnect microphone)
\item Record fault occurrence timestamp ($t_{fault}$) at moment of disconnection
\item Monitor diagnostic logs for error detection
\item Record diagnostic log entry timestamp ($t_{log}$)
\item Calculate detection latency: $\Delta t = t_{log} - t_{fault}$
\item Verify $\Delta t \leq 1$ second
\item Verify error code matches expected fault type (cross-reference with fault
injection log)
\end{enumerate}

The test passes if all faults are detected within timing constraints, with
correct error identification, zero false positives, and verified correspondence
between logged states and actual hardware states.
...
denied error codes without executing hardware operations. 
...
all access decisions match configured permissions. System logs will show
...
specific error conditions can be triggered (e.g., invalid GPIO pin specified).
					
\textbf{Input:}
Inject fault by changing source code to: (1) write to invalid GPIO pin number,
(2) configure timer with out-of-range parameters, (3) read from disconnected
peripheral.
					
\textbf{Output:}
For each error condition, the driver returns a specific error code identifying
the failure type (e.g., HAL\_RCC\_OSCILLATOR\_INIT\_FAIL,
HAL\_SAI\_A1\_INIT\_FAIL, HAL\_PCD\_INIT). Error codes are
returned immediately without hanging or crashing. 
...
\textbf{How test will be performed:}
Modify source code that inject specific faults.Execute test sequence and
capture return codes from driver API calls. Verify each return code matches
...
corruption occurs. No data races
...
at low (100 Hz), mid (1 kHz), and high (7.5 kHz) frequency ranges. No filtering or
...
100 Hz, 1 kHz, and 7.5 kHz, corresponding to the sine waves.
...
original audio clip, there are 3 sine waves at 100 Hz, 1 kHz, and 7.5 kHz. After
...
Hz, 1 kHz and 7.5 kHz. The test passes if all 3 peaks are present with no
...
accelerated run should complete at least 50\% faster than the non-accelerated
run.

\textbf{Pass Criteria:}
\begin{itemize}
\item Amplitude error $\leq$ 0.1\% for all frequency bins
\item Hardware-accelerated execution time $\leq$ 0.5 $\times$ non-accelerated execution time
\end{itemize}
...
verify that the pass criteria are met (hardware-accelerated execution time is at
least 50\% faster).
...
sine wave with an amplitude that exceeds 1.0. Another clip will have a 1 second 
sine wave, that is replaced by zeros halfway. The last clip will just have
2 seconds of zero amplitude.
...
and high (7.5 kHz) frequency ranges. No error flags are set in the input data.
...
rate. The input data will have error flags set, indicating either clipping, 
or lost signal.
...
either clipping, or lost signal.
...
module's classification accuracy can be evaluated. The 85\% accuracy requirement
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR1_1]{NFR1.1}):}
\begin{itemize}
\item 100\% monotonically increasing frame timestamps
\item Zero out-of-order frame processing events
\item Verified over full 60-second capture period
\end{itemize}
...
to verify all pass criteria are met (timestamps are monotonically increasing).
The test passes if no out-of-order processing is detected.
...
(e.g., processes 16,000 samples in less than 1 second). No buffer overflows
occur. No frames are dropped due to processing delays.

\textbf{Pass Criteria (from \hyperref[SRS-NFR1_2]{NFR1.2}):}
\begin{itemize}
\item Processing rate $\geq$ 16,000 samples/second for entire test duration
\item Zero buffer overflows
\item Zero dropped frames
\item Verified over 5-minute continuous operation
\end{itemize}
...
buffer overflow indicators in logs. 
...
\textbf{Output/Result:} 
For all N error-triggering calls, the driver returns error codes immediately
without delay or queueing. The median error propagation time is less than 1
millisecond, and the maximum propagation time is less than 10 milliseconds. All
errors are reported to the firmware layer immediately without buffering.

\textbf{Pass Criteria:}
\begin{itemize}
\item Maximum error propagation delay $<$ 10 ms for all calls
\item Median error propagation delay $<$ 1 ms
\item No errors are buffered or queued
\end{itemize}
...
statistics (minimum, median, maximum, 99th percentile). The test passes if all
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR3_1]{NFR3.1}):}
\begin{itemize}
\item Mean relative error $<$ 10\% across all frequency bins
\item Verified against high-precision FFT reference
\item Automated test passes on every commit
\end{itemize}
...
the following formula across all bins.
...
(processing must keep pace with 16 kHz input rate, i.e., process 16,000 samples
per second or faster).

\textbf{Pass Criteria:}
\begin{itemize}
\item Processing rate $\geq$ 16,000 samples/second for all input sizes
\item No frame drops or buffer overflows detected
\end{itemize}
...
input size meets the pass criteria (processing rate $\geq$ 16,000 
samples/second).
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR3_3]{NFR3.3}):}
\begin{itemize}
\item Mean relative error $<$ 10\% across all frequency bins
\item Verified over 60-second continuous audio sample
\item Comparison against high-precision reference implementation
\end{itemize}
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR4_1]{NFR4.1}):}
\begin{itemize}
\item Zero write operations to read-only input buffers
\item No data corruption in audio filtering module buffers
\item Static analysis reports no memory safety violations
\end{itemize}
...
writes to memory locations that are passed as read-only inputs. The test passes
if all pass criteria are met. This is important to ensure since the buffers used
by the audio filtering module are most likely serviced by interrupts. 
...
\textbf{Pass Criteria:}
\begin{itemize}
\item Processing time per frame $<$ 62.5 $\mu$s (1/16,000 seconds)
\item No frame drops over entire test duration
\item 100\% of frames processed within time constraint
\end{itemize}
...
verify that all pass criteria are met. This test will have to be performed on
the microcontroller to ensure accurate timing measurements.
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR4_3]{NFR4.3}):}
\begin{itemize}
\item No persistent storage of microphone data or derivatives
\item All audio buffers are cleared after processing
\item Code review confirms no data retention mechanisms
\end{itemize}
...
to ensure that all pass criteria are met and there are no persistent data
structures or variables that retain copies of the original microphone data or
any derivative data.
...
classes and directions for each source within defined accuracy thresholds.

\textbf{Pass Criteria (from \hyperref[SRS-NFR5_1]{NFR5.1}, \hyperref[SRS-NFR5_2]{NFR5.2}, \hyperref[SRS-NFR5_3]{NFR5.3}):}
\begin{itemize}
\item Classification accuracy $\geq$ 85\% per class
\item Direction estimation error $\leq$ 45° (azimuth angle)
\item Successfully processes up to 3 simultaneous sources
\end{itemize}
...
known classes and directions to verify all pass criteria are met.
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR5_2]{NFR5.2}):}
\begin{itemize}
\item Classification accuracy $\geq$ 85\% per audio class
\item Tested across all supported audio classes (siren, smoke alarm, speech, 
etc.)
\end{itemize}
...
the known audio samples to the Frequency Analysis module. The predicted classes
will be compared against the known classes to calculate the classification
accuracy. The test passes if accuracy $\geq$ 90\% for each class.
...
The Frequency Analysis module processes input samples from the four microphone
array and outputs the estimated direction of each audio source. The estimated
direction should match the known direction within the accuracy threshold.

\textbf{Pass Criteria (from \hyperref[SRS-NFR5_3]{NFR5.3}):}
\begin{itemize}
\item Direction estimation error $\leq$ 45° (azimuth angle on 2D plane)
\item Tested across multiple known source positions (0°, 45°, 90°, 135°, 180°, etc.)
\end{itemize}

\textbf{How test will be performed:}
The test will be performed automatically by feeding the frequency domain data of
the known direction samples to the Frequency Analysis module. The estimated
directions will be compared against the known directions to verify the error is
within 45° for all test cases.
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR6_1]{NFR6.1}):}
\begin{itemize}
\item Only highest priority source is displayed
\item Lower priority sources are suppressed
\item Verified across all priority levels (high, medium, low)
\end{itemize}
...
visually verifying that all pass criteria are met and only the highest priority
direction is visualized on the output display.
...
\textbf{Output/Result:} 
Stakeholders verify the non-obtrusive nature of the visualizations on the
...
\textbf{Pass Criteria (from \hyperref[SRS-NFR6_2]{NFR6.2}):}
\begin{itemize}
\item Average non-obtrusiveness rating $\geq$ 4.0 out of 5.0
\item Minimum 5 stakeholder participants
\item No reports of significant visual obstruction or discomfort
\end{itemize}
...
passes if all pass criteria are met.
...
to assess the effectiveness and usability of \progname{}.
...
\begin{enumerate}
    ...
    audio cues (siren approaching, alarms, etc.) that were identified as the
