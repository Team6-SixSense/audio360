...
2026-01-06 & 2.0 & Module type definitions \& formatting section Module
                   Decomposition.\\
2026-01-06 & 2.1 & Detailed timeline Rev0. \\
2026-03-07 & 3.0 & Timeline for individual contributions. \\
2026-03-07 & 3.1 & Uses hierarchy diagram labelling simplified. \\
2026-03-07 & 3.2 & Removed safety-critical secrete from visualization module.\\
2026-03-07 & 3.3 & Grammar and spelling fixes. \\
2026-03-07 & 3.4 & Including module name in table \ref{TblMH}.\\
2026-03-25 & 3.5 & Updating planned classification labels\\
\bottomrule
\end{tabularx}
...
\item[\refstepcounter{acnum} \actheacnum \label{SampleRate}:]
  ...
  their own. This may be necessary in scenarios where users would want to detect
  industrial machinery or animal communication which can emit high frequencies
...
\item[\refstepcounter{acnum} \actheacnum \label{SampleSize}:]
  ...
  \textbf{Rationale:} For higher quality of audio processing, a higher sample
  rate is required.
...
\item[\refstepcounter{acnum} \actheacnum \label{acConfidenceAction}:]
  ...
  shall update the classification label to unknown when the classification 
  is low confidence. However, this will be updated to the true classification 
  if the accuracy of the classification module is near perfect.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucClosed}:]
 ...
 \acref{acHardware} will be considered if additional compute power is required.
 \acref{acSDCardLogging} mentions about SD connected for logging during
 development only, however this unlikely change focuses on the final product.
...
& \mref{mMicInput}: Microphone Input Module \\ 
& \mref{mUSART}: USART Communication Module \\
& \mref{mSDInterface}: SD Card Interface Module \\
& \mref{mMicDiag}: Microphone Diagnostic Module \\
\midrule

\multirow{1}{0.3\textwidth}{Behaviour-Hiding Module}
& \mref{mAudio360Engine}: Audio360 Enginer Module \\
& \mref{mVisualization}: Visualization Module \\
\midrule

\multirow{16}{0.3\textwidth}{Software Decision Module}
& \mref{mAudioGen}: Audio Generation Module \\
& \mref{mDOA}: DOA Processor Module \\
& \mref{mAudioSampling}: Audio Sampling Module \\
& \mref{mAudioSpectralLeakage}: Audio spectral Leakage Prevention Module \\
& \mref{mAudioNormalizer}: Audio Normalizer Module \\
& \mref{mAudioAnomaly}: Audio Anomaly Detection Module \\
& \mref{mFFT}: Fast Fourier Transform Module \\
& \mref{mLogging}: Logging Module \\
& \mref{mFaultManager}: Fault Manager Module \\
& \mref{mMelFilter}: Mel Filter Module \\
& \mref{mDCT}: Discrete Cosine Transform Module\\
& \mref{mPCA}: Principle Component Analysis Module \\
& \mref{mLDA}: Linear Discriminant Analysis Module \\
& \mref{mClassification}: Classification Module \\
& \mref{mICA}: Independent Component Analysis Module \\
\bottomrule
...
the microphone diagnostic module \mref{mMicDiag} monitors the microphone array
for hardware faults.
...
\item \hyperref[SRS-FR5_1]{FR5.1}, \hyperref[SRS-FR5_4]{FR5.4} requires the
  ...
  de-coupled modules that will be called individually. This design choice
  affects \mref{mMelFilter}, \mref{mDCT}, \mref{mPCA}, \mref{mLDA} and
...
\newline
\newline
\noindent The \emph{type of module} field can be the following:
\begin{description}[before=\small, labelindent=0.5cm]
  \item [Library: ] Only export access programs, but do not have state
    variables.
  \item [Abstract Data Type: ] Modules that have both state and behaviour.
  \item [Abstract Object: ] Modules that have both state and behaviour but there
    can only be one instance.
  \item [Generic Modules: ] Modules that are either abstract data types or
    abstract objects and are parameterized by type.
  \item [Interface Modules: ] Modules that provides synteax of specification
...
microphone array hardware using hardware ADC. 
...
\item[Secrets:] The internal coordination logic that determines the execution
  ...
  It is also contains the prioritization logic for safety-critical information
  based on the data received from classification and DOA modules.
...
\begin{description}[labelindent=0.5cm]
  ...
  strategy for HUD elements, and the rendering pipeline for directional
  indicators, classification labels, and system faults.
  ...
  the smart glasses display. It also handles
...
\item[Services:] Analyzes four synchronized microphone audio streams to estimate
  ...
  frequency-domain audio signals and outputs an angle (in radians) representing
  ...
  accuracy of \hyperref[SRS-symbol:theta_e]{$\theta_e$} radian as specified in
  the SRS (\hyperref[SRS-NFR5_3]{NFR5.3}).
\item[Implemented By:] \progname{}
\item[Type of Module:] Abstract Data Type
...
\begin{description}[labelindent=0.5cm]
  ...
  and outputs the classification as known if it is. 
  \item[Implemented By:] \progname
  \item[Type of Module:] Abstract Object
\end{description}
...
\begin{description}[labelindent=0.5cm]
  ...
  audio streams, each representing an individual sound source.
  \item[Implemented By:] \progname
  \item[Type of Module] Abstract Object
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.34]{diagrams/uses_hierarchy.png}
\caption{Use hierarchy among modules. Software modules are green, hardware
  modules are blue and mock modules are light purple.}
\label{FigUH}
\end{figure}
\end{landscape}

Figure \ref{FigUH} has an ``output`` component. At this stage, the output of
the system is Smart glasses provided by Rokid \cite{RokidGlasses}. Rokid
provides an SDK for developers to interface with to visualize on the glasses
frame.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{../../Images/Smart glasses HUD.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{../../Images/Smart Glasses HUD Close Up.png}
...
\begin{itemize}
\item Microphone Input Module (\mref{mMicInput})
...
\item Audio Spectral Leakage Prevention Module (\mref{mAudioSpectralLeakage})
\item Fast Fourier Transform Module (\mref{mFFT})
\item Logging Module (\mref{mLogging})
\end{itemize}

For Revision 0, the team will complete the implementation of the
following modules listed in table \ref{table:rev0_timeline}.
These modules are required for Rev 0 because they support the core features of
the project which are audio classification and audio directional analysis.
Deadlines were assigned to ensure there is enough time for integration
verification with the hardware while respecting module dependencies. Each
module is assigned an owner. The owner is responsible for the main development
(but can delegate development to other if needed),
and managing any changes to the module for any Pull Requests that touches the
module's file. They are also the point of contact for testing purposes for the
module.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Module} & \textbf{Deadline} & \textbf{Module Owner} \\ \hline
        DOA (Direction of Arrival) Processor Module (\mref{mDOA})
          & Jan 23, 2026 & Kalp \\ 
        Audio360 Engine Module (\mref{mAudio360Engine}) & Jan 23, 2026
          & Sathurshan \\
        Audio Sampling Module (\mref{mAudioSampling}) & Jan 9, 2026
          & Omar \\
        Audio Normalizer Module (\mref{mAudioNormalizer}) & Jan 9, 2026
          & Sathurshan \\
        Audio Anomaly Detection Module (\mref{mAudioAnomaly}) & Jan 23, 2026
          & Omar \\
        Mel Filter Module (\mref{mMelFilter}) & Jan 9, 2026
          & Nirmal \\
        Discrete Cosine Transform Module (\mref{mDCT}) & Jan 9, 2026 
          & Nirmal \\
        Principle Component Analysis Module (\mref{mPCA}) & Jan 16, 2026
          & Nirmal \\
        Linear Discriminant Analysis Module (\mref{mLDA}) & Jan 16, 2026
          & Nirmal \\
        Classification Module (\mref{mClassification}) & Jan 16, 2026
          & Nirmal \\
        Visualization Module (\mref{mVisualization}) & Jan 16, 2026
          & Jay \\
        \hline
    \end{tabular}
    \caption{Rev 0 module development timeline.}
    \label{table:rev0_timeline}
\end{table}

Github issue $\#361$ to $\#375$ are created to track the implementation of 
modules. Note that modules implemented before POC may not explicity mention
"module" in the issue name as implementation finished before this document was
created. All issues have been assigned to team members based on their skills and
domain knowledge.

In parallel, the team will be writing unit tests for each module that is
written. This is to ensure that the software is being developed correctly as
there are limited number of microcontroller available to do full hardware
integration testing. Table \ref{table:rev0_testing_timeline} outlines the
deadline and tester lead for the essential modules. Unit tests and integration
testing must be complete by the deadline indicated in the table.
Omar Alam and Sathurshan Arulmohan will be taking main
responsibility in verifying core features such as audio classification and
audio direction analysis on hardware. 
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Module} & \textbf{Deadline} & \textbf{Tester} \\ \hline
        DOA (Direction of Arrival) Processor Module (\mref{mDOA})
          & Jan 30, 2026 & Sathurshan \\ 
        Audio360 Engine Module (\mref{mAudio360Engine}) & Jan 30, 2026
          & Omar \\
        Audio Sampling Module (\mref{mAudioSampling}) & Jan 15, 2026
          & Nirmal \\
        Audio Normalizer Module (\mref{mAudioNormalizer}) & Jan 15, 2026
          & Nirmal \\
        Audio Anomaly Detection Module (\mref{mAudioAnomaly}) & Jan 30, 2026
          & Sathurshan \\
        Mel Filter Module (\mref{mMelFilter}) & Jan 15, 2026
          & Jay \\
        Discrete Cosine Transform Module (\mref{mDCT}) & Jan 15, 2026 
          & Jay \\
        Principle Component Analysis Module (\mref{mPCA}) & Jan 25, 2026
          & Jay \\
        Linear Discriminant Analysis Module (\mref{mLDA}) & Jan 25, 2026
          & Kalp \\
        Classification Module (\mref{mClassification}) & Jan 25, 2026
          & Kalp \\
        Visualization Module (\mref{mVisualization}) & Jan 25, 2026
          & Omar \\
        \hline
    \end{tabular}
    \caption{Rev 0 module testing timeline.}
    \label{table:rev0_testing_timeline}
\end{table}
