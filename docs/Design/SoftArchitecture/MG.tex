...
\item[\refstepcounter{acnum} \actheacnum \label{acMicArrangement}:] 
  ...
  \textbf{Rationale:} The system may be deployed on other survaces other than
...
\item[\refstepcounter{acnum} \actheacnum \label{acHardwareAccel}:]
  ...
  tradional software algorithms. The trade-off of slower processing over
  power consumption may be desirable.

\item[\refstepcounter{acnum} \actheacnum \label{acConfidenceAction}:]
  Action when low confience occurs during audio source classification. \\
...
\item [\refstepcounter{mnum} \mthemnum \label{mAudioNormalizer}:] Audio
  Normalizer Module
\item [\refstepcounter{mnum} \mthemnum \label{mAudioAnamoly}:] Audio Anomoly
  Detection Module
...
% AUDIO ANOMOLY DETECTION MODULE.
\subsubsection{Audio Anomoly Detection Module (\mref{mAudioAnamoly})}
\begin{description}
...
\subsubsection{Fault Manager Module (\mref{mFaultManager})}
\begin{description}
\item[Secrets:] The internal mechanisms and decision logic used to detect,
  manage, and recover from system faults.
\item[Services:] Monitors system health and manages critical faults to maintain
  core functionality.
\item[Implemented By:] \progname
\end{description}
...
\midrule
\hyperref[SRS-FR1_4]{FR1.4} & \mref{mFaultManager} \\
...
\hyperref[SRS-FR3_5]{FR3.5} & \mref{mAudioAnamoly} \\
