...
\item[\refstepcounter{acnum} \actheacnum \label{acAudioGen}:] The method of 
  generating simulated microphone array input for testing and proof of
  concept. \\
  \textbf{Rationale:} Since the use case is primarily for testing and proof
  of concept, the team will be prioritize methods that are easily integratable
  and provide sufficient functionality of testing. This can change as the team
  experiments with different methods at earlier stages. The final product will
  replace module linked to this antricipated change entirely with hardware 
...
\item[\refstepcounter{acnum} \actheacnum \label{acICA}:]
  ...
  in complex acoustic environments. However it is complicated to implement
  and is defined as a stretch goal by the team.
\end{description}
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:]
 ...
 from the environment of the user. Although,~\acref{acAudioGen} mentions the
 potential of changing inputs for testing and proof of concept, \uref{ucInput}
 is focued on the final product.
...
\item \hyperref[SRS-FR5_2]{FR5.2}, \hyperref[SRS-NFR5_3]{NFR5.3} requires
  ...
  \mref{mDOA} is responsible for implementing these requirements. This
  modular design facilitates testing to ensure that the specified error
...
\item \hyperref[SRS-FR6_1]{FR6.1}, \hyperref[SRS-FR6_2]{FR6.2}, 
  ...
  presentation on the smart glasses display. The module implements
  prioritization 
...
\item The SRS mentions multi-source handling as a functionality goal
  ...
  are present simultaneously by separating them before processing. The module is
  designed 
...
\emph{\progname{}} means the module will be implemented by the
\progname{} software.
...
\item[Services:] Analyzes four synchronized microphone audio streams to estimate 
  ...
  accuracy of 45 degrees as specified in the SRS (\hyperref[SRS-NFR5_3]{NFR5.3}).
