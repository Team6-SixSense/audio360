...
\item [\refstepcounter{mnum} \mthemnum \label{mFaultManager}:] Fault Manager
  Module 
\item [\refstepcounter{mnum} \mthemnum \label{mSTFT}:] Short Time Fourier 
Transform Module
...
\item \hyperref[SRS-FR5_1]{FR5.1}, \hyperref[SRS-FR5_4]{FR5.4} requires the 
  ...
  \mref{mSTFT}, \mref{mMelFilter}, \mref{mDCT}, \mref{mPCA}, \mref{mLDA} and 
  \mref{mClassification}. 
...
\subsubsection{Short Time Fourier Transform Module (\mref{mSTFT})}
\begin{description}
  \item[Secrets:] The specific algorithm to compute the Short Time Fourier 
  Transform (STFT).
  \item[Services:] Accepts framed time-domain samples and outputs their 
  corresponding frequency-domain magnitude for each time frame. Involves 
  using \mref{mFFT} module to compute the FFT for each time frame. 
  \item[Implemented By:] \progname
\end{description}
...
\hline
\hyperref[SRS-FR5_1]{FR5.1} & \mref{mSTFT}, \mref{mMelFilter}, \mref{mDCT}, 
