...
\section{MIS of Microphone Input Module \label{MMicInput} - \mref{MG-mMicInput}}
...
\begin{enumerate}
  ...
  \item Microphone Diagnostic Module - \mref{MG-mMicDiag}
  \item \hyperref[ds:cb-mic-data]{CALLBACK\_MIC\_DATA} 
  \item SAI\_HandleTypeDef - STM32 HAL Library
\end{enumerate}
...
\section{MIS of USART Communication Module \label{MUsartComm} - \mref{MG-mUSART}}
\subsection{Module}
...
\subsection{Uses}
\begin{enumerate}
  \item USART Driver - STM32 HAL Library
  \item USART\_Init() - STM32 HAL Library
  \item UART\_Transmit() - STM32 HAL Library
\end{enumerate}
...
\newpage
\section{MIS of SD Card Interface Module \label{MIS:sdcard} - \mref{MG-mSDInterface}}
...
\subsection{Uses}
\begin{enumerate}
  \item SPI Driver - STM32 HAL Library
  \item FATFS Library - Chan's FatFs
\end{enumerate}
...
\section{MIS of Microphone Diagnostic Module \label{MIS:micdiag}- \mref{MG-mMicDiag}}
\subsection{Module}
...
\subsection{Uses}
\begin{itemize}
  \item ADC Driver - STM32 HAL Library
\end{itemize}
...
\section{MIS of Audio Anomaly Detection Module \label{MAudioAnomaly} - 
...
\subsection{Uses}
\begin{itemize}
  \item \hyperref[MMicInput]{Mic Input Module} 
\end{itemize}
...
\begin{itemize}
  \item currentDisplayState [\hyperref[generic:dict]{dict}]: 
  ...
  \item displayQueue [\hyperref[generic:list]{list}]: 
...
\begin{itemize}
  ...
  \item applyPrioritization(displayQueue: 
    \hyperref[generic:list]{list}) $\rightarrow$ 
    \hyperref[generic:dict]{dict}: 
  ...
  \item renderNonIntrusive(displayData: 
    \hyperref[generic:dict]{dict}) $\rightarrow$ 
    \hyperref[generic:bool]{bool}: 
...
\begin{itemize}
  \item separationModel [\hyperref[generic:dict]{dict}]: 
...
\begin{itemize}
  ...
  \item checkConvergence(previousModel: 
    \hyperref[generic:dict]{dict}, currentModel: 
    \hyperref[generic:dict]{dict}) $\rightarrow$ 
    \hyperref[generic:bool]{bool}: 
...
\begin{enumerate}
  ...
  write out this document, especailly given the lenght and detail that it goes 
  ...
  computation. Originally, I was using a SVM for classififying audio sources, 
  ...
  to serperate a process into multiple modules leads to more specific 
  requirements). 
  ...
  \textbf{Nirmal:} Another desgin solution we considered for classification was 
  ...
  blackbox models. The approach now just invovles doing matrix multiplications 
