...
\begin{enumerate}
  ...
  \item directionalAnalsysiFault: \label{enum:FaultState:DOAFault} Fault with
...
\hyperref[generic:string]{string} & micrphoneData[][]:
...
\item \textbf{exception:} Invalid spectrogram dimension. If the dimension of the
  ...
  computted FFT is not equal to the number of frequency bins in the pre-computed
...
\item \textbf{output:} Returns a matrix \textit{pca(t, K)}, where $t$ represents
  ...
  retained (proportional to the number of eignevectors). Essentially, each input
...
This is module is optimized for class seperation. The feature vector in this
...
\begin{itemize}
  ...
  subspace where class seperation is maximized.
...
\item \textbf{output:}
  ...
  class $j$, where $w_j$ and $b_j$ is the weight and bias matricies that were
...
\item \textbf{exception:} Invalid feature space dimension. If the dimension of
  ...
  the number of eignvectors in the precomputed LDA matrix.
\end{itemize}
...
\begin{itemize}
  ...
  continuously be updated at runtime after recieving continuous audio signals. 
