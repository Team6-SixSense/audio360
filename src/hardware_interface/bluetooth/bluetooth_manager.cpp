...
void Bluetooth_Manager_Init() {
  ...
  memset(bluetooth_manager.data_buffer, 0, sizeof(bluetooth_manager.data_buffer));
...
void Bluetooth_Manager_Process()
{
  ...
  if (bluetooth_manager.state == BLUETOOTH_IDLE) {
    ...
    HAL_UART_Transmit(bluetooth_manager.uart_handle, (uint8_t*)cmd, strlen(cmd), 100);

    // Clear buffer
    memset(bluetooth_manager.data_buffer, 0, sizeof(bluetooth_manager.data_buffer));

    // 4 bytes for "OK\r\n")
    HAL_UART_Receive(bluetooth_manager.uart_handle, bluetooth_manager.data_buffer, 4, 1000);
...
HAL_StatusTypeDef Bluetooth_Manager_Send(uint8_t *data) {
  return HAL_UART_Transmit(bluetooth_manager.uart_handle, data, strlen((char*)data), 100);
}